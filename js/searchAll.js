...
var vm = function () {
    ...
    //---Variáveis locais
    var self = this;
    self.countryp=ko.observableArray([]);
    self.athletep=ko.observableArray([]);
    self.modalityp=ko.observableArray([]);
    self.competitionp=ko.observableArray([]);
    ...
    self.calculatePercentage=function(){
        ...
        var temp=setInterval(function(){
            for (set of self.all()){
                if (set.TableName=='Athletes') self.athletep().push(set)
                else if (set.TableName=='Countries') self.countryp().push(set)
                else if (set.TableName=='Modalities') self.modalityp().push(set)
                else self.competitionp().push(set)
            }
            self.athletep(self.athletep().sort(function(a,b){
                if (a.Name>b.Name) return 1
                if (a.Name<b.Name) return -1
                return 0
            }))
            self.countryp(self.countryp().sort(function(a,b){
                if (a.Name>b.Name) return 1
                if (a.Name<b.Name) return -1
                return 0
            }))
            self.modalityp(self.modalityp().sort(function(a,b){
                if (a.Name>b.Name) return 1
                if (a.Name<b.Name) return -1
                return 0
            }))
            self.competitionp(self.competitionp().sort(function(a,b){
                if (a.Name>b.Name) return 1
                if (a.Name<b.Name) return -1
                return 0
            }))
